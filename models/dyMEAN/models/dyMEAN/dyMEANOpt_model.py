...
class dyMEANOptModel(nn.Module):
    ...
    def _noMask_forward(self, X, S, cmask, smask, residue_pos, init_noise=None, fixbb=True):
        batch_id = self.batch_constants['batch_id']
    ...
    def get_seq_nlls(self, X, S, cmask, smask, residue_pos, lengths, return_hidden=False, init_noise=None, fixbb=True):
        ...
        # generate
        H, pred_S, r_pred_S_logits, pred_X, _ = self._noMask_forward(X, S, cmask, smask, residue_pos, init_noise, fixbb)

        # PPL
        #print(f"structure only:{self.struct_only}")
        if not self.struct_only:
            #print(f"calculate sequence negative log-likelihood: fix backbone={fixbb}")
            
            # only mask position
            #S_logits = r_pred_S_logits[-1][0][smask]
            #print(f"S_logits={S_logits}")
            #S_dists = torch.softmax(S_logits, dim=-1)
            #pred_S[smask] = torch.multinomial(S_dists, num_samples=1).squeeze()
            #print(f"pred_S[smask]={pred_S[smask]}")
            ...
            #print(f"-torch.log(S_probs)={-torch.log(S_probs)}")
            #ppl = scatter_mean(nlls, s_batch_id)  # [batch_size]
            #nlls = torch.mean(nlls)

            # average over all positions
            # S_logits represents the raw predictions (logits) for the sequences
            # sequence negtive log likelihood
            nlls_list = []  # To store NLL values for each position

            for pos in range(r_pred_S_logits[-1][0].shape[0]):  # Iterate over all positions
                # Extract logits for the current position
                S_logits = r_pred_S_logits[-1][0][pos]
                #print(f"S_logits (position {pos}) = {S_logits}")
                
                # Compute distribution
                S_dists = torch.softmax(S_logits, dim=-1)
                
                # Sample predicted amino acid/token
                pred_S[pos] = torch.multinomial(S_dists, num_samples=1).squeeze()
                #print(f"pred_S[{pos}] = {pred_S[pos]}")
                
                # Get probability of the sampled token
                S_probs = S_dists[pred_S[pos]]
                #print(f"S_probs (position {pos}) = {S_probs}")
                
                # Compute negative log-likelihood
                nll = -torch.log(S_probs)
                nlls_list.append(nll)
                #print(f"Negative log-likelihood (position {pos}) = {nll}")

            # Compute mean NLL over all positions
            mean_nll = torch.mean(torch.stack(nlls_list))  # Convert list to tensor for mean calculation
            nlls = mean_nll
            #print(f"Mean Negative Log-Likelihood: {mean_nll}")

            
        else:
            ppl = torch.zeros(batch_size, device=pred_S.device)
            nlls = torch.zeros(batch_size, device=pred_S.device) # add placeholder for nlls (yct)
    ...
    def sample(self, X, S, cmask, smask, residue_pos, lengths, return_hidden=False, init_noise=None):
        ...
        if return_hidden:
            return gen_X, gen_S, ppl, H
        return gen_X, gen_S, nlls
        #return gen_X, gen_S, ppl # test (yct)
    ...
    def optimize_sample(self, X, S, cmask, smask, residue_pos, lengths, predictor, opt_steps=10, init_noise=None, mask_only=False):
        ...
        for i in range(opt_steps):
            ...
            # test print nlls
            gen_X, gen_S, nlls, H = self.sample(X, S, cmask, smask, residue_pos, lengths, return_hidden=True, init_noise=all_noise[cmask])
        ...
        # test print nlls#
        return final_X, final_S, best_metric, nlls
