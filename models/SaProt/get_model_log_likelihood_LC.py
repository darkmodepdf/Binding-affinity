...
def calc_fitness(foldseek_bin, model, data, tokenizer,
    ...
    log_proba_list = []
    for mut_info in tqdm(data[mutation_col], total=len(data), desc="Calculating fitness scores"):
        try:
            score = get_ll_average_complex(model, tokenizer, seq, mut_info)
        except:
            score = None
            print(f"Error processing mutation info: {mut_info}")
...
def main():
    ...
    for csv_file in affinity_data_files:
        ...
        # 5b) Ensure 'heavy_chain_seq' is present
        if "heavy_chain_seq" not in df.columns:
            print(f"'heavy_chain_seq' col missing in {csv_file}, skipping.")
            continue
        if "light_chain_seq" not in df.columns:
            print(f"'light_chain_seq' col missing in {csv_file}, skipping.")
            continue

        # 5c) Build 'mutations' column
        df["mutations"] = df.apply(
            lambda row: identify_mutations(wt_hc+wt_lc, row["heavy_chain_seq"]+row["light_chain_seq"])
                        if isinstance(row["heavy_chain_seq"], str) and isinstance(row["light_chain_seq"], str) else None,
            axis=1
        )
