...
def design_for_pdb(args):
    ...
    for variant in data_variants:
        ...
        for batch in tqdm(loader, desc=variant['name'], dynamic_ncols=True):
            ...
            LL_per_sequence = traj_batch[0][3] # Extract log-likelihood per sequence (yct)
            ...
            record_list = [] # save sequence, log-likelihood to file (yct)
            for i in range(aa_new.size(0)):
                ...
                # append to list
                method = f"{config.mode}_{config.sampling.seed}"
                ll_per_sequence = LL_per_sequence[i].item()  # Convert the tensor to a scalar
                record_list.append( ('%04d.pdb' % (count, ), method, ll_per_sequence) )
            ...
            csv_path = os.path.join(log_dir, variant['tag'], 'sequence_loglikelihoods.csv')
            df = pd.DataFrame(record_list, columns=['pdb id', 'sequence', 'loglikelihood'])
