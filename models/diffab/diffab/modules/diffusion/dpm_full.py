...
def compute_log_likelihood(post, s_next, epsilon=1e-9):
    """
    Compute the log-likelihood for a batch of sequences.
    
    Args:
        post: Posterior probabilities, shape (N, L, K)
        s_next: Sampled sequence, shape (N, L)
        epsilon: Small constant for numerical stability.
        
    Returns:
        LL_per_sequence: Log-likelihood per sequence, shape (N,)
        LL_batch: Total log-likelihood for the batch, scalar.

    Ref: Exploring Log-Likelihood Scores for Ranking Antibody Sequence Designs (Talip Uçar Cedric Malherbe Ferran Gonzalez)
    """
    # Extract probabilities of sampled amino acids
    N, L, K = post.size()
    probs = post[torch.arange(N).unsqueeze(1), torch.arange(L), s_next]  # Shape: (N, L)
    
    # Add numerical stability
    probs = probs + epsilon

    # Compute log probabilities
    log_probs = torch.log(probs)  # Shape: (N, L)

    # Sum log probabilities across sequence positions
    LL_per_sequence = log_probs.sum(dim=1)  # Shape: (N,)

    # Compute total log-likelihood for the batch
    LL_batch = LL_per_sequence.sum()  # Scalar

    return LL_per_sequence, LL_batch
...
class FullDPM(nn.Module):
    ...
    def forward(self, v_0, p_0, s_0, res_feat, pair_feat, mask_generate, mask_res, denoise_structure, denoise_sequence, t=None):
        ...
        # add log-likelihood by yct
        log_likelihood = torch.sum(post_true * log_post_pred, dim=-1).mean() 
        loss_dict['loglikelihood'] = log_likelihood
        return loss_dict
    ...
    def sample(
        ...
        # Initialize trajectory and log-likelihood tracking
        traj = {self.num_steps: (v_init, self._unnormalize_position(p_init), s_init, None)}
        if pbar:
            pbar = functools.partial(tqdm, total=self.num_steps, desc='Sampling')
        else:
            pbar = lambda x: x
        for t in pbar(range(self.num_steps, 0, -1)):
            v_t, p_t, s_t, _ = traj[t]  # Include log-likelihood from previous step
            ...
            post, s_next = self.trans_seq.denoise(s_t, c_denoised, mask_generate, t_tensor)

            # Calculate log-likelihood
            LL_per_sequence, LL_batch = compute_log_likelihood(post, s_next, epsilon=1e-9)
            ...
            traj[t-1] = (v_next, self._unnormalize_position(p_next), s_next, LL_per_sequence)
            traj[t] =  tuple(x.cpu() if isinstance(x, torch.Tensor) else x for x in traj[t])  #tuple(x.cpu() for x in traj[t])    # Move previous states to cpu memory.
    ...
    def optimize_diffab(
        ...
        traj = {opt_step: (v_init, self._unnormalize_position(p_init), s_init, None)}
        if pbar:
    ...
    def optimize(
        ...
        # Initialize trajectory and log-likelihood tracking
        traj = {opt_step: (v_init, self._unnormalize_position(p_init), s_init, None)}  # Initialize LL as None
        ...
        for t in pbar(range(opt_step, 0, -1)):
            ...
            # Update trajectory with log-likelihood
            traj[t-1] = (v_next, self._unnormalize_position(p_next), s_next, LL_per_sequence)
            
            # Move previous states to CPU memory to save GPU memory
            traj[t] = tuple(x.cpu() if isinstance(x, torch.Tensor) else x for x in traj[t])  
